...
__device__ void ModSub256isOdd(uint64_t* r, uint64_t* a, uint64_t* b, uint8_t* parity) {
    ...
    // Determinazione della parit� in base al LSB di T[0] e al borrow
    *parity = (T[0] & 1) ^ (t & 1);  // LSB di T[0] e LSB di t determinano la parit�

}

__device__ void ModSub256isOdd(uint64_t* r, uint64_t* b, uint8_t* parity) {

    uint64_t t;   // Variabile che conterr� il borrow finale
    uint64_t T[4]; // Variabile temporanea per il calcolo del borrow e del risultato

    // Sottrazione con borrow per ciascun blocco a 64 bit
    USUBO(T[0], r[0], b[0]);   // Sottrazione con borrow per il primo blocco
    USUBC(T[1], r[1], b[1]);   // Sottrazione con carry (continuazione) per il secondo blocco
    USUBC(T[2], r[2], b[2]);   // Sottrazione con carry (continuazione) per il terzo blocco
    USUBC(T[3], r[3], b[3]);   // Sottrazione con carry (continuazione) per il quarto blocco

    // USUB calcola la sottrazione tra 0 e 0, quindi t contiene il borrow finale
    USUB(t, 0ULL, 0ULL);  // Calcola il borrow finale
...
__device__ __noinline__ void _ModInvGrouped256(uint64_t r[256 / 2 + 1][4]) {
    ...
    Load256(subp[0], r[0]);
    for (uint32_t i = 1; i < (256 / 2 + 1); i++) {
        _ModMult(subp[i], subp[i - 1], r[i]);
    }

    // We need 320bit signed int for ModInv
    Load256(inverse, subp[(256 / 2 + 1) - 1]);
    inverse[4] = 0;
    _ModInv(inverse);

    for (uint32_t i = (256 / 2 + 1) - 1; i > 0; i--) {
        _ModMult(newValue, subp[i - 1], inverse);
        _ModMult(inverse, r[i]);
...
__device__ __noinline__ void _ModInvGrouped2048(uint64_t r[2048 / 2 + 1][4]) {
    ...
    Load256(subp[0], r[0]);
    for (uint32_t i = 1; i < (2048 / 2 + 1); i++) {
        _ModMult(subp[i], subp[i - 1], r[i]);
    }

    // We need 320bit signed int for ModInv
    Load256(inverse, subp[(2048 / 2 + 1) - 1]);
    inverse[4] = 0;
    _ModInv(inverse);

    for (uint32_t i = (2048 / 2 + 1) - 1; i > 0; i--) {
        _ModMult(newValue, subp[i - 1], inverse);
        _ModMult(inverse, r[i]);
